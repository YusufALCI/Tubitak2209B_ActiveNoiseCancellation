/*
 * anc_core.c - FxLMS FINAL ENGINE (Fixed & Secured)
 * Revizyon: Latency Compensation, Fast RNG, Debug Report & SAFETY CLAMPING
 * Düzeltmeler: Printf kilitlenmesi çözüldü, Saturation eklendi.
 */
...
// --- DEBUG DEĞİŞKENLERİ (Yusuf'un Ekledikleri) ---
volatile float32_t debug_calib_error = 0.0f;     // İkincil Yol Doğruluğu
volatile float32_t debug_anc_noise_level = 0.0f; // FxLMS Başarımı (Gürültü Seviyesi)
...
// --- XORSHIFT RNG (Hızlı Rastgele Sayı Üreteci) ---
...
void ANC_Core_Init(void)
{
    ...
    wn_index = 0;
    hist_index = 0;
    debug_calib_error = 0.0f;
    debug_anc_noise_level = 0.0f;

    rng_state = 2463534242;
}

// --- CYCLE HANDLER & RAPORLAMA (DÜZELTİLDİ) ---
void ANC_Cycle_Handler(void)
{
    // Ses işleme bloğunu çağır
    if (active_buffer != NULL) process_audio_block();

    // FFT işleme (Sadece Running modunda)
    if (anc_state == ANC_STATE_RUNNING && is_fft_ready == 1) {
        Find_Dominant_Freq();
        is_fft_ready = 0;
    }

    // A) Durum Değişikliği Tespiti (Sadece geçişlerde yazar)
    if (anc_state != prev_anc_state)
    {
        switch (anc_state)
        {
            ...
            case ANC_STATE_RUNNING:
                printf("\n>>> DURUM: ANC RUNNING (Guc Kalkanlari Devrede) <<<\n");
                break;
        }
        prev_anc_state = anc_state;
    }

    // B) Periyodik Canlılık Raporu (Saniyede 1 Kez - DÜZELTİLDİ)
    // Tüm printf işlemleri BU BLOK içinde olmalı!
    if (HAL_GetTick() - last_report_time > 1000)
    {
        if (anc_state == ANC_STATE_RUNNING)
        {
            // Tüm önemli verileri tek satırda yazalım ki SWV şişmesin
            printf("[ANC] Freq: %4.0f Hz | NoiseLvl: %.5f | W[0]: %.5f | SecPath[0]: %.5f\n",
                   current_freq,
                   debug_anc_noise_level,
                   anc_coeffs[0],
                   sec_path_coeffs[0]);
        }
        else if (anc_state == ANC_STATE_CALIBRATION)
        {
            printf("[CALIB] Error Lvl: %.6f | SecPath[0]: %.5f | Sample: %lu\n",
                   debug_calib_error,
                   sec_path_coeffs[0],
...
void process_audio_block(void)
{
    ...
    // Mikrofon kazancı (Giriş sinyalini güçlendirmek için)
    ...
    else
    {
        ...
        for(int i=0; i < NUM_SAMPLES; i++)
        {
            ...
            // 2. Filtrelenmiş Referans (x' = S_hat * x)
            ...
            // 3. Anti-Noise Üret (y = W * x)
            float32_t y_out = 0.0f;
            for(int k=0; k < ANC_FILTER_LENGTH; k++) {
                y_out += anc_coeffs[k] * GET_HIST(x_history, hist_index, k);
            }

            // 4. Hata (e) - Mikrofondan gelen
            float32_t error = input_buffer[i];

            // FxLMS doğruluk takibi
            debug_anc_noise_level = (0.99f * debug_anc_noise_level) + (0.01f * fabsf(error));

            // 5. [GÜVENLİ] LMS GÜNCELLEMESİ (W = W + mu * e * x')
            if(current_volume > 0.01f) {
                for(int k=0; k < ANC_FILTER_LENGTH; k++) {
                    ...
                    // Sızıntı (Leakage) - Filtreyi dizginler
                    anc_coeffs[k] *= ANC_LEAKAGE;

                    // Katsayı Sınırlama (Clamp) - Patlamayı önler
                    if (anc_coeffs[k] > 2.0f) anc_coeffs[k] = 2.0f;
                    else if (anc_coeffs[k] < -2.0f) anc_coeffs[k] = -2.0f;
                }
            }

            // [EK GÜVENLİK] Çıkış Sınırlama (Saturation)
            // Sistem patlasa bile hoparlöre giden sinyali keser.
            if (y_out > 0.8f) y_out = 0.8f;
            if (y_out < -0.8f) y_out = -0.8f;

            // Hoparlör Kazancı (Patlama riskine karşı dikkatli ol)
            output_buffer[i] = y_out * 2.0f; // Yeni değişti
        }
    }

    float_to_i2s_stereo(output_buffer, i2s_dest, NUM_SAMPLES);

    // FFT İçin Buffer Doldurma
    if (is_fft_ready == 0) {
    ...
    // İşimiz bitti, buffer'ı serbest bırak
    active_buffer = NULL;
}
